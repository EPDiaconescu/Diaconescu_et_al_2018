...
list_varName=['rx1day','rx5days']
periode='1980to2004'
ValnrYY='15'
fyy='1980'
lyy='2004'
output='D:/NowWorking/newProjet/results/domARC_CORDEX_b/KuiperPerkins_anom2_sptp_ref/'
input='D:/NowWorking/newProjet/results/domARC_CORDEX_b/simSelected_ref/'
outputMean= 'D:/NowWorking/newProjet/results/domARC_CORDEX_b/simSelected_climMean_ref/'
...
for varName in list_varName:
    ...
    listMM=['nrcan_canada_', 'gmfd_reanalysis_', 'cfsr_reanalysis_','merra_reanalysis_','jra55_reanalysis_', 'eraint_reanalysis_',
            'AWI-HIRHAM5_ARC44_MPI-ESM-LR_histo_', 'CanRCM4_ARC22_CanESM2_histo_','CanRCM4_ARC44_CanESM2_histo_',
            'CRCM5_ARC44_MPI-ESM-LR_histo_','CRCM5_NA44_MPI-ESM-LR_histo_','CRCM5_NA44_CanESM2_histo_','RCA4_ARC44_CanESM2_histo_',
            'RCA4_ARC44_NorESM1-M_histo_','RCA4_ARC44_EC-EARTH_histo_','RCA4SN_ARC44_EC-EARTH_histo_','RCA4_ARC44_MPI-ESM-LR_histo_',
            'RCA4SN_ARC44_MPI-ESM-LR_histo_']
    listMM2=['Obs', 'NRCan', 'GMFD', 'CFSR', 'MERRA', 'JRA55', 'ERAI', 'AWI-HIRHAM5-MPI-ESM-LR', 'CCCma-CanRCM4-CanESM2-022',
             'CCCma-CanRCM4-CanESM2','UQAM-CRCM5-MPI-ESM-LR', 'UQAM-CRCM5NA-MPI-ESM-LR', 'UQAM-CRCM5NA-CanESM2',
             'SMHI-RCA4-CanESM2', 'SMHI-RCA4-NorESM1-M', 'SMHI-RCA4-EC-EARTH', 'SMHI-RCA4SN-EC-EARTH','SMHI-RCA4-MPI-ESM-LR','SMHI-RCA4SN-MPI-ESM-LR']
    ...
    meanFinal.to_csv(outputMean+varName+'ClimaticMeanMatrix_'+domeniu+'_'+str(ValnrYY)+'noYY_'+periode+'_sptpSelected.csv', sep=',')
    ...
    for mm in range(1,model):
        ...
        ax.plot( ecdfX.x, ecdfX.y, color='blue',label="Obs; D="+str(D)[:4])
#        ax.fill_between(ecdfX.x, 0, ecdfX.y, color='blue', alpha=.3)
        ax.plot( ecdfY.x, ecdfY.y, color='green', label=newFrame.columns[mm])
#        ax.fill_between(ecdfY.x, 0, ecdfY.y, color='green', alpha=.3)
        plt.yticks(fontsize=14)
        plt.xticks(fontsize=14)
        plt.legend(loc=4, fontsize=14)
        plt.savefig(output+'figuresECDF/'+varName+'AnualMean_ecdfKuiper_'+newFrame.columns[mm]+'_'+domeniu+'_'+str(ValnrYY)+'noYY_'+periode+'_spTempo.png')
        fig1.clf()
        ...
        ax.patch.set_facecolor('white')
        ax.bar(centerX, histX, align='center', width=widthX, fc='blue',alpha=0.3)
        ax.bar(centerY, histY, align='center', width=widthY, fc='green',alpha=0.3)
        ax.plot( centerX, testM2, 'k--',linewidth=3,label="bin="+str(centerX[1]-centerX[0])+"; P="+str(areaMin2)[:4])
        plt.yticks(fontsize=14)
        plt.xticks(fontsize=14)
        plt.legend(loc=1, fontsize=14)
        plt.savefig(output+'figuresHistogrames/'+varName+'AnualMean_PerkinsHisto_'+newFrame.columns[mm]+'_'+domeniu+'_'+str(ValnrYY)+'noYY_'+periode+'_spTemp.png')
        fig2.clf()


    Dval_matrix=pd.concat({'Kuiper': pd.Series(Dval), 'PerkinsH': pd.Series(1-Pkval)}, axis=1)
    Dval_matrix.index=listMM2
    Dval_matrix.to_csv(output+'KuiperPerkinsVal/'+varName+'AnualMean_KuiperPerkinsMatrix_'+domeniu+'_'+str(ValnrYY)+'noYY_'+periode+'_sptpAnom.csv', sep=',')
